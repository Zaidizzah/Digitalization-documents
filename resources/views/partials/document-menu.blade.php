...
<nav class="nav-container mb-3" id="document-menu">
    <ul class="nav-menu">
        <li class="nav-item">
            <a href="{{ route($name ? 'documents.files.index' : 'documents.files.root.index', $name) }}" class="nav-link {{ set_active($name ? 'documents.files.index' : 'documents.files.root.index') }}" aria-label="Manage files" title="Manage files">
