    ...
    <div class="tile" aria-label="Tile section of schema attributes for document type {{ $document_type->name }}" aria-labelledby="tile-schema-document-type-label">
        <div class="tile-title-w-btn flex-wrap">  
            <div class="tile-title flex-nowrap">
                <h3 class="title" id="tile-schema-document-type-label">Schema attributes for document type {!! $document_type->abbr ?? $document_type->name !!}</h3>
            ...
            <div class="tile-buttons">
                ...
                <a href="{{ route("documents.schema.reorder.page", $document_type->name) }}" class="btn btn-primary btn-sm" id="btn-change-order-schema-attributes-document-type" title="Button: to change the order of schema attributes for document type {{ $document_type->name }}"><i class="bi bi-sort-numeric-down fs-5"></i> Change Order</a>
